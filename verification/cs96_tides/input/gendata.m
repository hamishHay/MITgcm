...
%- take grid files from CVS in contrib:
%  > cvs co -P -d grid_cs96 MITgcm_contrib/dyncore_ASP/csgrids/ref_96
kwr=[1 1 1 1]*0;
 kwr=[0 1 0 0];
prec='real*4';
...
nc=G.dims(2); nPxy=G.dims(1)*G.dims(2); nPp2=nPxy+2;
...
% fid=fopen(fname,'w','b'); fwrite(fid,wallW,prec); fclose(fid);
% fprintf(['write file: ',fname,'\n']);
% fname='wall_S_DbD.bin';
% fid=fopen(fname,'w','b'); fwrite(fid,wallS,prec); fclose(fid);
% fprintf(['write file: ',fname,'\n']);
%end
...
if kwr(2) > 0,
 ...
 var=ones(nPxy,1);
 var=qBot*var;
%fname=sprintf('%s%3.3i%s','Q_bottom_',round(qBot/1000),'kW.bin');
 fname=sprintf('%s%3.1f%s','Q_bottom_',qBot,'W.bin');
 fid=fopen(fname,'w','b'); fwrite(fid,var,prec); fclose(fid);
 fprintf(['write file: ',fname,'\n']);
end 
...
if kwr(4) > 0,
%delTemp=0.530; % in Kelvin
%delTemp=1.;
%sfx=sprintf('%2.2i%s',delTemp,'K.bin');
 delTemp=0.25;
 dTemp=round(delTemp*1000); % im mili-Kelvin
 sfx=sprintf('%3i%s',dTemp,'mK.bin');
%--
 var=zeros(nPxy,nr);
 var(:,nr)=var(:,nr)+delTemp;
 fname=['rbcs_T+',sfx];
 fid=fopen(fname,'w','b'); fwrite(fid,var,prec); fclose(fid);
 fprintf(['write file: ',fname,'\n']);
...
%- for initial temperature: start from the same Temp field as for RBCS
 var=zeros(nPxy,nr);
 var(:,nr)=var(:,nr)+delTemp;
%- and add small noise at the bottom
 dTnoise=1.e-3;
 dTn=rand([nPxy 1]); dTn=dTn-mean(dTn);
 var(:,nr)=var(:,nr)+dTnoise*dTn;
 fname=['theta_ini.',sfx];
 fid=fopen(fname,'w','b'); fwrite(fid,var,prec); fclose(fid);
 fprintf(['write file: ',fname,'\n']);
end
